class B77RS_APU extends B77RS_APUInfo {

	constructor() {
		super(...arguments);
		this.apuEGT = SimVar.GetSimVarValue('AMBIENT TEMPERATURE', 'celsius');
		this.apuRPM = SimVar.GetSimVarValue(B77RS_LocalVariables.APU.RPM, 'Percent');
		this.apuLastRPM = this.apuRPM;
		this.apuSwitchPosition = SimVar.GetSimVarValue(B77RS_LocalVariables.APU.SWITCH_STATE, 'Bool');
	...
	getApuRPM(over100 = false) {
		if (over100 === true) {
			return SimVar.GetSimVarValue(B77RS_LocalVariables.APU.RPM, 'Percent over 100');
		}
		return SimVar.GetSimVarValue(B77RS_LocalVariables.APU.RPM, 'Percent');
	}
