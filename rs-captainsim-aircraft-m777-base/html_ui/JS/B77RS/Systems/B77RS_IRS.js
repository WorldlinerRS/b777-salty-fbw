...
class B77RS_IRS {
	...
	init() {
		this.irsLState = SimVar.GetSimVarValue('L:B77RS_IRS_L_STATE', 'Number');
		this.irsRState = SimVar.GetSimVarValue('L:B77RS_IRS_R_STATE', 'Number');
		this.irsLSwitchState = SimVar.GetSimVarValue('L:B77RS_IRS_L_SWITCH_STATE', 'Number');
		this.irsRSwitchState = SimVar.GetSimVarValue('L:B77RS_IRS_R_SWITCH_STATE', 'Number');
		this.isIrsInited = SimVar.GetSimVarValue('L:B77RS_IS_IRS_INITED', 'Number');
	}

	checkAlignStates() {
		if (this.irsLSwitchState === 0) {
			SimVar.SetSimVarValue('L:B77RS_IRS_L_STATE', 'Number', 0);
			SimVar.SetSimVarValue('L:B77RS_IRS_L_INIT_ALIGN_TIME', 'Number', -1);
			this.initLAlignTime = null;
			this.irsLState = 0;
		}

		if (this.irsRSwitchState === 0) {
			SimVar.SetSimVarValue('L:B77RS_IRS_R_STATE', 'Number', 0);
			SimVar.SetSimVarValue('L:B77RS_IRS_R_INIT_ALIGN_TIME', 'Number', -1);
			this.initRAlignTime = null;
			this.irsRState = 0;
		}
	...
	executeInit() {
		if (this.irsLSwitchState > 0 && this.irsLState < 1) {
			SimVar.SetSimVarValue('L:B77RS_IRS_L_STATE', 'Number', 1);
			this.irsLState = 1;
			if(!this.isIrsInited){
				this.isIrsInited = Math.floor(new Date().getTime() / 1000);
				SimVar.SetSimVarValue('L:B77RS_IS_IRS_INITED', 'String', this.isIrsInited.toString());
			}
		}

		if (this.irsRSwitchState > 0 && this.irsRState < 1) {
			SimVar.SetSimVarValue('L:B77RS_IRS_R_STATE', 'Number', 1);
			this.irsRState = 1;
			if(!this.isIrsInited){
				this.isIrsInited = Math.floor(new Date().getTime() / 1000);
				SimVar.SetSimVarValue('L:B77RS_IS_IRS_INITED', 'String', this.isIrsInited.toString());
	...
	executeIRSDeinit() {
		...
		SimVar.SetSimVarValue('L:B77RS_IRS_L_STATE', 'Number', 0);
		SimVar.SetSimVarValue('L:B77RS_IRS_R_STATE', 'Number', 0);
		SimVar.SetSimVarValue('L:B77RS_IS_IRS_INITED', 'Number', 0);
	}
		...
		if (this.irsLSwitchState > 0) {
			if (this.irsLState !== 2) {
				if (!this.initLAlignTime) {
					...
					SimVar.SetSimVarValue('L:B77RS_IRS_L_INIT_ALIGN_TIME', 'String', nowSeconds.toString());
					SimVar.SetSimVarValue('L:B77RS_IRS_L_STATE', 'Number', 1);
				} else {
					if (this.initLAlignTime + this.irsLTimeForAligning < this.initLAlignTime) {
						SimVar.SetSimVarValue('L:B77RS_IRS_L_STATE', 'Number', 2);
					}
				}
			}
		} else {
		...
		if (this.irsRSwitchState > 0) {
			if (this.irsRState !== 2) {
				if (!this.initRAlignTime) {
					...
					SimVar.SetSimVarValue('L:B77RS_IRS_R_INIT_ALIGN_TIME', 'String', nowSeconds.toString());
					SimVar.SetSimVarValue('L:B77RS_IRS_R_STATE', 'Number', 1);
				} else {
					if (this.initRAlignTime + this.irsRTimeForAligning < nowSeconds) {
						SimVar.SetSimVarValue('L:B77RS_IRS_R_STATE', 'Number', 2);
					}
				}
			}
		} else {
	...
	updateVariables() {
		...
		this.isIrsPositionSet = SimVar.GetSimVarValue('L:B77RS_IS_IRS_POSITION_SET', 'Boolean');
	}
	...
	generateTimeForAligning(irsId, minimal, maximal) {
		...
		if (!minimal && !maximal) {
			switch (HeavyDataStorage.get('IRS_ALIGN_SPEED', B77RS_IRS.ALIGN_SPEED.REAL)) {
				case B77RS_IRS.ALIGN_SPEED.INSTANT:
					minimal = 0;
					maximal = 0;
					break;
				case B77RS_IRS.ALIGN_SPEED.FAST:
					minimal = 70;
					maximal = 110;
					break;
				case B77RS_IRS.ALIGN_SPEED.NORMAL:
					minimal = 230;
					maximal = 270;
					break;
				case B77RS_IRS.ALIGN_SPEED.REAL:
					let sqr = 0;
					let timeToAlign = 0;
					let timeSec = 0;
		...
		SimVar.SetSimVarValue('L:B77RS_IRS_' + irsId + '_TIME_FOR_ALIGN', 'Number', ret);
		return ret;
	}
}
