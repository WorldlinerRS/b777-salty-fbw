(function (global, factory) {
  ...
  class B777_FMC_IdentPage$1 {
    static ShowPage1(fmc) {
      fmc.cleanUpPage();
      B777_FMC_IdentPage$1._updateCounter = 0;
      fmc.pageUpdate = () => {
        if (B777_FMC_IdentPage$1._updateCounter >= 50) {
          B777_FMC_IdentPage$1.ShowPage1(fmc);
        } else {
          B777_FMC_IdentPage$1._updateCounter++;
        }
      };
  ...
  _defineProperty(B777_FMC_IdentPage$1, "_updateCounter", 0);
  ...
  class B777_FMC_PayloadManagerPage$1 {
    ...
    init() {
      this.tankPriorityValues = [{
        'LEFT_MAIN': this.getTankValue(B777_FMC_PayloadManagerPage$1.tankVariables.LEFT_MAIN),
        'RIGHT_MAIN': this.getTankValue(B777_FMC_PayloadManagerPage$1.tankVariables.RIGHT_MAIN)
      }, {
        'CENTER': this.getTankValue(B777_FMC_PayloadManagerPage$1.tankVariables.CENTER)
      }];
      this._internalPayloadValuesCache = [];
      this.payloadValues = this.getPayloadValues();
      B777_FMC_PayloadManagerPage$1.centerOfGravity = this.getCenterOfGravity();
    }
    getPayloadValues() {
      return [{
        "PILOT": this.getPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.PILOT),
        "COPILOT": this.getPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.COPILOT),
        "CREW": this.getPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.CREW)
      }, {
        "BUSINESS_CLASS": this.getPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.BUSINESS_CLASS),
        "PREMIUM_ECONOMY": this.getPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.PREMIUM_ECONOMY),
        "FORWARD_BAGGAGE": this.getPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.FORWARD_BAGGAGE)
      }, {
        "FORWARD_ECONOMY": this.getPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.FORWARD_ECONOMY),
        "REAR_ECONOMY": this.getPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.REAR_ECONOMY),
        "REAR_BAGGAGE": this.getPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.REAR_BAGGAGE)
      }];
    }
    ...
    async flushFuelAndPayload() {
      return new Promise(resolve => {
        this.flushFuel().then(() => {
          ...
          return this.fmc.setZeroFuelWeight((299000 + B777_FMC_PayloadManagerPage$1.requestedPayload) / 1000, EmptyCallback.Void, true);
        }).then(() => {
          return this.resetPayload();
        }).then(() => {
          resolve();
    ...
    async flushFuel() {
      return new Promise(resolve => {
        ...
        B777_FMC_PayloadManagerPage$1.tankPriority.forEach((tanks, index) => {
          tanks.forEach(tank => {
            setTankFuel(B777_FMC_PayloadManagerPage$1.tankVariables[tank], 0).then(() => {
              console.log(B777_FMC_PayloadManagerPage$1.tankVariables[tank] + ' flushed');
            });
          });
        });
        this.fmc.trySetBlockFuel(0, true);
    ...
    calculateTanks(fuel) {
      ...
      B777_FMC_PayloadManagerPage$1.tankPriority.forEach((tanks, index) => {
        tanks.forEach(tank => {
          setTankFuel(B777_FMC_PayloadManagerPage$1.tankVariables[tank], this.tankPriorityValues[index][tank]).then(() => {
            console.log(B777_FMC_PayloadManagerPage$1.tankVariables[tank] + ' set to ' + this.tankPriorityValues[index][tank]);
          });
        });
      });
    ...
    calculateMainTanks(fuel) {
      let remainingFuel = 0;
      let tanksCapacity = B777_FMC_PayloadManagerPage$1.tankCapacity.LEFT_MAIN * 2;
    ...
    calculateCenterTank(fuel) {
      let remainingFuel = 0;
      let tankCapacity = B777_FMC_PayloadManagerPage$1.tankCapacity.CENTER;
    ...
    showPage() {
      this.fmc.cleanUpPage();
      this.payloadValues = this.getPayloadValues();
      if (!B777_FMC_PayloadManagerPage$1.requestedPayload) {
        B777_FMC_PayloadManagerPage$1.requestedPayload = this.getTotalPayload(true);
      }
      if (!B777_FMC_PayloadManagerPage$1.requestedCenterOfGravity) {
        B777_FMC_PayloadManagerPage$1.requestedCenterOfGravity = this.getCenterOfGravity();
      }
      if (!B777_FMC_PayloadManagerPage$1.requestedFuel) {
        B777_FMC_PayloadManagerPage$1.requestedFuel = this.getTotalFuel();
      }
      if (B777_FMC_PayloadManagerPage$1.isPayloadManagerExecuted) {
        this.fmc.pageUpdate = () => {
          this.showPage();
        };
      }
      ...
      const fobReqToRender = B777_FMC_PayloadManagerPage$1.requestedFuel ? (B777_FMC_PayloadManagerPage$1.requestedFuel * weightPerGallon).toFixed(2) : fobToRender;
      const totalPayload = this.getTotalPayload(useImperial);
      const payloadToRender = totalPayload.toFixed(0);
      const payloadReqToRender = B777_FMC_PayloadManagerPage$1.requestedPayload ? (B777_FMC_PayloadManagerPage$1.requestedPayload * payloadModifier).toFixed(0) : payloadToRender;
      B777_FMC_PayloadManagerPage$1.requestedFuel ? B777_FMC_PayloadManagerPage$1.requestedFuel.toFixed(2) : this.getTotalFuel().toFixed(2);
      rows[0][0] = 'REQ VALUES';
      rows[0][1] = 'ACT VALUES';
      rows[2][0] = 'CG';
      rows[2][1] = 'CG';
      rows[3][0] = B777_FMC_PayloadManagerPage$1.requestedCenterOfGravity ? B777_FMC_PayloadManagerPage$1.requestedCenterOfGravity.toFixed(2) + '%' : B777_FMC_PayloadManagerPage$1.centerOfGravity.toFixed(2) + '%';
      ...
      rows[8][0] = B777_FMC_PayloadManagerPage$1.remainingPayload ? 'REMAINING PAYLOAD' : '';
      rows[9][0] = B777_FMC_PayloadManagerPage$1.remainingPayload ? B777_FMC_PayloadManagerPage$1.remainingPayload + ' lb' : '';
      rows[11][0] = '<BACK';
      this.fmc._renderer.lsk(2).event = () => {
        if (isFinite(parseFloat(this.fmc.inOut))) {
          if (parseFloat(this.fmc.inOut) > B777_FMC_PayloadManagerPage$1.getMinCenterOfGravity && parseFloat(this.fmc.inOut) < B777_FMC_PayloadManagerPage$1.getMaxCenterOfGravity) {
            B777_FMC_PayloadManagerPage$1.requestedCenterOfGravity = parseFloat(this.fmc.inOut);
            this.fmc.clearUserInput();
            this.showPage();
          } else {
      ...
      this.fmc._renderer.lsk(3).event = () => {
        if (isFinite(parseFloat(this.fmc.inOut))) {
          ...
          if (parseFloat(requestedInGallons) > B777_FMC_PayloadManagerPage$1.getMinFuel && parseFloat(requestedInGallons) < B777_FMC_PayloadManagerPage$1.getMaxFuel) {
            B777_FMC_PayloadManagerPage$1.requestedFuel = parseFloat(requestedInGallons);
            this.fmc.clearUserInput();
            this.showPage();
          } else {
            this.fmc.showErrorMessage('OUT OF RANGE');
      ...
      this.fmc._renderer.lsk(4).event = () => {
        if (isFinite(parseFloat(this.fmc.inOut))) {
          ...
          if (parseFloat(requestedInPounds) > B777_FMC_PayloadManagerPage$1.getMinPayload && parseFloat(requestedInPounds) < B777_FMC_PayloadManagerPage$1.getMaxPayload) {
            B777_FMC_PayloadManagerPage$1.requestedPayload = parseFloat(requestedInPounds);
            this.fmc.clearUserInput();
            this.showPage();
          } else {
            this.fmc.showErrorMessage('OUT OF RANGE');
      ...
      if (B777_FMC_PayloadManagerPage$1.isPayloadManagerExecuted) {
        ...
        this.fmc._renderer.rsk(6).event = () => {
          B777_FMC_PayloadManagerPage$1.isPayloadManagerExecuted = true;
          this.flushFuelAndPayload().then(() => {
            if (B777_FMC_PayloadManagerPage$1.requestedFuel) {
              this.calculateTanks(B777_FMC_PayloadManagerPage$1.requestedFuel);
            } else {
              this.calculateTanks(this.getTotalFuel());
            }
            if (B777_FMC_PayloadManagerPage$1.requestedPayload) {
              this.calculatePayload(B777_FMC_PayloadManagerPage$1.requestedPayload).then(() => {
                B777_FMC_PayloadManagerPage$1.isPayloadManagerExecuted = false;
              });
            } else {
    ...
    async calculatePayload(requestedPayload) {
      ...
      B777_FMC_PayloadManagerPage$1.remainingPayload = requestedPayload;
      let amount = 0;
      let requestedCenterOfGravity = B777_FMC_PayloadManagerPage$1.requestedCenterOfGravity ? B777_FMC_PayloadManagerPage$1.requestedCenterOfGravity : this.getCenterOfGravity();
      while (B777_FMC_PayloadManagerPage$1.remainingPayload > 0) {
        B777_FMC_PayloadManagerPage$1.centerOfGravity = this.getCenterOfGravity();
        if (B777_FMC_PayloadManagerPage$1.remainingPayload > 30000) {
          amount = 1000;
        } else if (B777_FMC_PayloadManagerPage$1.remainingPayload > 10000) {
          amount = 200;
        } else if (B777_FMC_PayloadManagerPage$1.remainingPayload > 5000) {
          amount = 100;
        } else if (B777_FMC_PayloadManagerPage$1.remainingPayload > 50) {
          amount = 50;
        } else {
          amount = B777_FMC_PayloadManagerPage$1.remainingPayload;
        }
        if (B777_FMC_PayloadManagerPage$1.centerOfGravity > requestedCenterOfGravity) {
          await this.increaseFrontPayload(amount, requestedCenterOfGravity);
          B777_FMC_PayloadManagerPage$1.remainingPayload = B777_FMC_PayloadManagerPage$1.remainingPayload - amount;
        } else {
    ...
    async increaseFrontPayload(amount, requestedCenterOfGravity) {
      ...
      let randomFront;
      let actualValue;
      if (B777_FMC_PayloadManagerPage$1.centerOfGravity > requestedCenterOfGravity + 0.05) {
        actualValue = this.getPayloadValueFromCache(B777_FMC_PayloadManagerPage$1.payloadIndex.BUSINESS_CLASS);
        await this.setPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.BUSINESS_CLASS, amount + actualValue);
      } else if (B777_FMC_PayloadManagerPage$1.centerOfGravity > requestedCenterOfGravity + 0.01) {
        randomFront = keys[Math.floor(Math.random() * keys.length)];
        actualValue = this.getPayloadValueFromCache(B777_FMC_PayloadManagerPage$1.payloadIndex[randomFront]);
        await this.setPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex[randomFront], amount + actualValue);
      } else {
        actualValue = this.getPayloadValueFromCache(B777_FMC_PayloadManagerPage$1.payloadIndex.PREMIUM_ECONOMY);
        await this.setPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.PREMIUM_ECONOMY, amount + actualValue);
      }
    }
    async increaseRearPayload(amount, requestedCenterOfGravity) {
      ...
      let actualValue;
      if (B777_FMC_PayloadManagerPage$1.centerOfGravity < requestedCenterOfGravity - 0.05) {
        actualValue = this.getPayloadValueFromCache(B777_FMC_PayloadManagerPage$1.payloadIndex.REAR_ECONOMY);
        await this.setPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.REAR_ECONOMY, amount + actualValue);
      } else if (B777_FMC_PayloadManagerPage$1.centerOfGravity < requestedCenterOfGravity - 0.01) {
        randomRear = keys[Math.floor(Math.random() * keys.length)];
        actualValue = this.getPayloadValueFromCache(B777_FMC_PayloadManagerPage$1.payloadIndex[randomRear]);
        await this.setPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex[randomRear], amount + actualValue);
      } else {
        actualValue = this.getPayloadValueFromCache(B777_FMC_PayloadManagerPage$1.payloadIndex.FORWARD_ECONOMY);
        await this.setPayloadValue(B777_FMC_PayloadManagerPage$1.payloadIndex.FORWARD_ECONOMY, amount + actualValue);
      }
    }
  }
  _defineProperty(B777_FMC_PayloadManagerPage$1, "isPayloadManagerExecuted", undefined);
  _defineProperty(B777_FMC_PayloadManagerPage$1, "centerOfGravity", undefined);
  _defineProperty(B777_FMC_PayloadManagerPage$1, "requestedCenterOfGravity", null);
  _defineProperty(B777_FMC_PayloadManagerPage$1, "requestedFuel", null);
  _defineProperty(B777_FMC_PayloadManagerPage$1, "requestedPayload", null);
  _defineProperty(B777_FMC_PayloadManagerPage$1, "remainingPayload", null);
  ...
  class B777_FMC_RobPage {
    static ShowPage1(fmc) {
      ...
      if (!B777_FMC_RobPage.WITHOUT_MANAGERS) {
        ...
        fmc._renderer.rsk(2).event = () => {
          new B777_FMC_PayloadManagerPage$1(fmc).showPage();
        };
        /*
         fmc._renderer.rsk(3).event = () => {
    ...
    */
    ...
    Init() {
      ...
      */
      ...
      B777_FMC_IdentPage$1.ShowPage1(this);
    }
    onPowerOn() {
      super.onPowerOn();
      this.deactivateLNAV();
  ...
  class B777_FMC_IdentPage {
    static ShowPage1(fmc) {
      ...
      let date = fmc.getNavDataDateRange();
      fmc._renderer.renderTitle('IDENT');
      fmc._renderer.render([['MODEL', 'ENG RATING'], ['777-300.2', 'GE90-115BL'], ['NAV DATA', 'ACTIVE'], ['AIRAC', date.toString()], ['OP PROGRAM', 'CO DATA'], [fmc.fmcManVersion, 'VS1001'], ['OPC'], [fmc.fmcBakVersion, ''], ['', 'DRAG/FF'], ['', '+0.0/+0.0'], ['', '__FMCSEPARATOR', ''], ['<INDEX', 'POS INIT>']]);
      if (fmc.urlConfig.index == 1) {
        fmc._renderer.lsk(6).event = () => {
          B777_FMC_InitRefIndexPage.ShowPage1(fmc);
        };
  ...
  // Payload Manager for B777-300ER
    ...
    async flushFuelAndPayload() {
      return new Promise(resolve => {
        this.flushFuel().then(() => {
          ...
          return this.fmc.setZeroFuelWeight((370000 + B777_FMC_PayloadManagerPage.requestedPayload) / 1000, EmptyCallback.Void, true);
        }).then(() => {
          return this.resetPayload();
        }).then(() => {
          resolve();
