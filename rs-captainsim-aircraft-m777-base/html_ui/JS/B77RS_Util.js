...
B77RS_Util.createDeltaTimeCalculator = (startTime = Date.now()) => {
...
B77RS_Util.createFrameCounter = (interval = 5) => {
    let count = 0;
    return () => {
        const c = count++;
        if (c == interval) {
...
B77RS_Util.createMachine = (machineDef) => {
    const machine = {
 ...
 */
B77RS_Util.trueToMagnetic = (heading, magVar) => {
 ...
 */
B77RS_Util.magneticToTrue = (heading, magVar) => {
    return (720 + heading + (magVar || SimVar.GetSimVarValue("MAGVAR", "degree"))) % 360;
};
 ...
 */
B77RS_Util.latLonToSpherical = (ll) => {
    return [
 ...
 * @returns {LatLong}
 */
B77RS_Util.sphericalToLatLon = (s) => {
 ...
 * @returns {LatLong}
 */
B77RS_Util.greatCircleIntersection = (latlon1, brg1, latlon2, brg2) => {
    ...
    const s1 = B77RS_Util.sphericalToLatLon(I1);
    const s2 = B77RS_Util.sphericalToLatLon(I2);
...
B77RS_Util.bothGreatCircleIntersections = (latlon1, brg1, latlon2, brg2) => {
    // c.f. https://blog.mbedded.ninja/mathematics/geometry/spherical-geometry/finding-the-intersection-of-two-arcs-that-lie-on-a-sphere/
    const Pa11 = B77RS_Util.latLonToSpherical(latlon1);
    const latlon12 = Avionics.Utils.bearingDistanceToCoordinates(brg1 % 360, 100, latlon1.lat, latlon1.long);
    const Pa12 = B77RS_Util.latLonToSpherical(latlon12);
 ...
 */
B77RS_Util.getIsaTemp = (alt = Simplane.getAltitude()) => {
 ...
 */
B77RS_Util.getIsaTempDeviation = (alt = Simplane.getAltitude(), sat = Simplane.getAmbientTemperature()) => {
    return sat - B77RS_Util.getIsaTemp(alt);
};
...
B77RS_Util.UpdateThrottler = UpdateThrottler;
...
class NXPopUp {
    constructor() {
        ...
        this.params.buttons.push(new NotificationButton("TT:MENU.YES", "B77RS_POP_" + this.params.id + "_YES"));
        this.params.buttons.push(new NotificationButton("TT:MENU.NO", "B77RS_POP_" + this.params.id + "_NO"));
    }
    ...
    showPopUp(title, message, style, callbackYes, callbackNo) {
        ...
        if (callbackYes) {
            ...
            Coherent.on(`B77RS_POP_${this.params.id}_YES`, () => {
                Coherent.off(`B77RS_POP_${this.params.id}_YES`, null, null);
                yes();
            });
        }
        if (callbackNo) {
            const no = (typeof callbackNo === "function") ? callbackNo : () => callbackNo;
            Coherent.on(`B77RS_POP_${this.params.id}_NO`, () => {
                Coherent.off(`B77RS_POP_${this.params.id}_NO`, null, null);
                no();
            });
        }
...
B77RS_Util.meterToFeet = (meterValue) => {
