 ...
 */
    ...
    updateHalfwayToDest() {
        ...
        if (destination && waypointActive) {
            ...
            if (destinationDistance < destination.cumulativeDistanceInFP / 2 || destinationDistance < 400) {
                SimVar.SetSimVarValue("L:77S_FMC_PASSED_HALFWAY", "bool", true);
            }
            else {
                SimVar.SetSimVarValue("L:77S_FMC_PASSED_HALFWAY", "bool", false);
            }
        }
        else {
            SimVar.SetSimVarValue("L:77S_FMC_PASSED_HALFWAY", "bool", false);
    ...
    updateAltitudeAlerting() {
        ...
        const afdsPitchMode = SimVar.GetSimVarValue("L:77S_PITCH_MODE_ACTIVE", "number");
        ...
        if (parkBrake == 100 || afdsPitchMode == 9 || (flapsPos >= 25 && gearPos == 100)) {
            ...
            SimVar.SetSimVarValue("L:77S_ALT_ALERT", "enum", 0);
            return;
        }
        ...
        if ((mcpAlt > alt && vSpeed > 150) || (mcpAlt < alt && vSpeed < -150)) {
            if (Math.abs(mcpAlt - alt) <= 900 && Math.abs(mcpAlt - alt) >= 200) {
                SimVar.SetSimVarValue("L:77S_ALT_ALERT", "enum", 1);
            }
        }
        else if (Math.abs(vSpeed) > 150){
            if (Math.abs(mcpAlt - alt) <= 900 && Math.abs(mcpAlt - alt) >= 200) {
                SimVar.SetSimVarValue("L:77S_ALT_ALERT", "enum", 2);
            }
        }

        //Reset Condition
        const alertState = SimVar.GetSimVarValue("L:77S_ALT_ALERT", "enum");
        if (alertState !== 0) {
