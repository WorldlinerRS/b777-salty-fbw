    ...
    recalculateTHRRedAccTransAlt() {
        ...
        if (origin) {
            if (isFinite(origin.infos.oneWayRunways[0].elevation)) {
                ...
                SimVar.SetSimVarValue("L:AIRLINER_THR_RED_ALT", "Number", this.thrustReductionAltitude);
                SimVar.SetSimVarValue("L:AIRLINER_ACC_ALT", "Number", this.accelerationAltitude);
                SimVar.SetSimVarValue("L:77S_FMC_ORIGIN_ELEVATION", "Number", altitude);
            }
        }
        else {
            ...
            SimVar.SetSimVarValue("L:77S_FMC_ORIGIN_ELEVATION", "Number", -1);
        }
        let destination = this.flightPlanManager.getDestination();
        if (destination) {
            if (destination.infos instanceof AirportInfo) {
                ...
                if (isFinite(origin.infos.oneWayRunways[0].elevation)) {
                    SimVar.SetSimVarValue("L:77S_FMC_DEST_ELEVATION", "Number", Math.round(destination.infos.oneWayRunways[0].elevation * 3.28 / 10) * 10);
                }
            }
        }
        else {
            SimVar.SetSimVarValue("L:77S_FMC_DEST_ELEVATION", "Number", -1);
        }
    }
    onPowerOn() {
        super.onPowerOn();
