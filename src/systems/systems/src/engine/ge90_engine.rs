...
impl GE90Engine {
    // According to the Type Certificate Data Sheet of GE90-110B1, GE90-113B, GE90-115B
    // Max N2 rpm is 121.0% @ 11292 RPM
    // 100% @ 9332 RPM
    const GE90_110B1_MAX_N2_RPM: f64 = 9332.0;
    // Gear ratio from primary gearbox input to EDP drive shaft
    const PUMP_N2_GEAR_RATIO: f64 = 0.3783;

    const MIN_IDLE_N2_UNCORRECTED_THRESHOLD_PERCENT: f64 = 55.;

    const LOW_OIL_PRESSURE_THRESHOLD_PSI: f64 = 18.;

    pub fn new(context: &mut InitContext, number: usize) -> GE90Engine {
        GE90Engine {
    ...
    fn update_parameters(&mut self) {
        self.n2_speed = AngularVelocity::new::<revolution_per_minute>(
            self.uncorrected_n2.get::<percent>() * Self::GE90_110B1_MAX_N2_RPM / 100.,
        );
...
impl SimulationElement for GE90Engine {
...
impl EngineCorrectedN1 for GE90Engine {
    fn corrected_n1(&self) -> Ratio {
        self.corrected_n1
    }
}
impl EngineCorrectedN2 for GE90Engine {
    fn corrected_n2(&self) -> Ratio {
        self.corrected_n2
    }
}
impl EngineUncorrectedN2 for GE90Engine {
    fn uncorrected_n2(&self) -> Ratio {
        self.uncorrected_n2
    }
}
impl Engine for GE90Engine {
    fn hydraulic_pump_output_speed(&self) -> AngularVelocity {
        self.hydraulic_pump_output_speed
    }

    fn oil_pressure_is_low(&self) -> bool {
        self.oil_pressure.get::<psi>() < GE90Engine::LOW_OIL_PRESSURE_THRESHOLD_PSI
    }

    fn is_above_minimum_idle(&self) -> bool {
        self.uncorrected_n2
            >= Ratio::new::<percent>(GE90Engine::MIN_IDLE_N2_UNCORRECTED_THRESHOLD_PERCENT)
    }
}
