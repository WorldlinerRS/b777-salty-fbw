...
impl PackFlowControllers<3> for B777AirConditioning {
...
impl SimulationElement for B777AirConditioning {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
        self.b777_cabin.accept(visitor);
...
impl B777Cabin {
    ...
    const B777_CABIN_VOLUME_CUBIC_METER: f64 = 200.; // m3
    const B777_COCKPIT_VOLUME_CUBIC_METER: f64 = 10.; // m3
        ...
        Self {
            cabin_zone: [
                CabinZone::new(
                    context,
                    ZoneType::Cockpit,
                    Volume::new::<cubic_meter>(Self::B777_COCKPIT_VOLUME_CUBIC_METER),
                    2,
                    None,
                ),
                CabinZone::new(
                    context,
                    ZoneType::Cabin(1),
                    Volume::new::<cubic_meter>(Self::B777_CABIN_VOLUME_CUBIC_METER / 2.),
                    0,
                    Some([(1, 6), (7, 13)]),
                ),
                CabinZone::new(
    ...
    fn update(
        ...
        let flow_rate_per_cubic_meter: MassRate = MassRate::new::<kilogram_per_second>(
            pack_flow.pack_flow().get::<kilogram_per_second>()
                / (Self::B777_CABIN_VOLUME_CUBIC_METER + Self::B777_COCKPIT_VOLUME_CUBIC_METER),
        );
...
impl SimulationElement for B777Cabin {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
