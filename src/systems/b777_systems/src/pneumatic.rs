...
impl B777Pneumatic {
    ...
    pub(crate) fn update(
        ...
        overhead_panel: &B777PneumaticOverheadPanel,
...
impl PneumaticBleed for B777Pneumatic {
...
impl EngineStartState for B777Pneumatic {
...
impl PackFlowValveState for B777Pneumatic {
...
impl SimulationElement for B777Pneumatic {
...
impl ReservoirAirPressure for B777Pneumatic {
...
pub struct B777PneumaticOverheadPanel {
...
impl B777PneumaticOverheadPanel {
    pub fn new(context: &mut InitContext) -> Self {
        B777PneumaticOverheadPanel {
...
impl EngineBleedPushbutton for B777PneumaticOverheadPanel {
    fn engine_bleed_pushbuttons_are_auto(&self) -> [bool; 2] {
        [self.engine_1_bleed.is_auto(), self.engine_2_bleed.is_auto()]
    }
}
impl SimulationElement for B777PneumaticOverheadPanel {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
...
/// This is a unique valve (and specific to the B777 probably) because it is controlled by two motors. One for manual control and one for automatic control
...
mod tests {
    ...
    use super::{B777Pneumatic, B777PneumaticOverheadPanel};

    struct TestAirConditioning {
        b777_air_conditioning_system: AirConditioningSystem<3>,
        adirs: TestAdirs,
    ...
    impl TestAirConditioning {
        fn new(context: &mut InitContext) -> Self {
            ...
            Self {
                b777_air_conditioning_system: AirConditioningSystem::new(
                    context,
                    cabin_zones,
        ...
        fn update(
            ...
            self.b777_air_conditioning_system.update(
                context,
                &self.adirs,
                engines,
                engine_fire_push_buttons,
    ...
    struct PneumaticTestAircraft {
        ...
        engine_1: GE90Engine,
        engine_2: GE90Engine,
        pneumatic_overhead_panel: B777PneumaticOverheadPanel,
    ...
    impl PneumaticTestAircraft {
        fn new(context: &mut InitContext) -> Self {
            Self {
                ...
                pneumatic_overhead_panel: B777PneumaticOverheadPanel::new(context),
    ...
    fn full_graphing_test() {
        ...
        if fs::create_dir_all("../b777_pneumatic_simulation_graph_data").is_ok() {
            let mut file = File::create("../b777_pneumatic_simulation_graph_data/generic_data.txt")
    ...
    fn hydraulic_reservoir_pressurization_graphs() {
        ...
        if fs::create_dir_all("../b777_pneumatic_simulation_graph_data").is_ok() {
            let mut file = File::create(
                "../b777_pneumatic_simulation_graph_data/hydraulic_reservoir_pressures_data.txt",
            )
            .expect("Could not create file");
    ...
    fn pack_pressurization_graphs() {
        ...
        if fs::create_dir_all("../b777_pneumatic_simulation_graph_data").is_ok() {
            let mut file =
                File::create("../b777_pneumatic_simulation_graph_data/pack_pressures_data.txt")
                    .expect("Could not create file");
