...
import { FlightArea } from '@aims/flightplanning/FlightPlanManager';
import { FlightPlanManager } from '@aims/index';
...
export class RequiredPerformance {
    ...
    private setActiveRnp(rnp: number): void {
        this.activeRnp = rnp;
        SimVar.SetSimVarValue('L:B77RS_AIMS_L_RNP', 'number', rnp ?? 0);
        SimVar.SetSimVarValue('L:B77RS_AIMS_R_RNP', 'number', rnp ?? 0);
    }
    ...
    private updateLDev(): void {
        ...
        if (ldev !== this.requestLDev) {
            ...
            SimVar.SetSimVarValue('L:B77RS_AIMS_L_LDEV_REQUEST', 'bool', this.requestLDev);
            SimVar.SetSimVarValue('L:B77RS_AIMS_R_LDEV_REQUEST', 'bool', this.requestLDev);
        }
    }
}
