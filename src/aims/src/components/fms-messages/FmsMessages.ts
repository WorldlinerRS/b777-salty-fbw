...
import { TurnAreaExceedanceLeft, TurnAreaExceedanceRight } from '@aims/components/fms-messages/TurnAreaExceedance';
...
export class FmsMessages implements FmgcComponent {
    ...
    update(deltaTime: number): void {
        ...
        if (didMutateNd) {
            for (const side in this.ndMessageFlags) {
                if (Object.prototype.hasOwnProperty.call(this.ndMessageFlags, side)) {
                    SimVar.SetSimVarValue(`L:B77RS_EFIS_${side}_ND_FM_MESSAGE_FLAGS`, 'number', this.ndMessageFlags[side]);
                }
            }
        }
    }
