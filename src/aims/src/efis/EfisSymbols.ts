...
export class EfisSymbols {
    ...
    async update(deltaTime: number): Promise<void> {
        ...
        for (const side of EfisSymbols.sides) {
            const range = rangeSettings[SimVar.GetSimVarValue(`L:B77RS_EFIS_${side}_ND_RANGE`, 'number')];
            const mode: Mode = SimVar.GetSimVarValue(`L:B77RS_EFIS_${side}_ND_MODE`, 'number');
            const efisOption = SimVar.GetSimVarValue(`L:B77RS_EFIS_${side}_OPTION`, 'Enum');
            ...
            if (mode === Mode.PLAN && !planCentre) {
                this.syncer.sendEvent(`B77RS_EFIS_${side}_SYMBOLS`, []);
                return;
            }
            ...
            this.syncer.sendEvent(`B77RS_EFIS_${side}_SYMBOLS`, symbols);
