...
import { PathVector, pathVectorLength, pathVectorValid } from '@aims/guidance/lnav/PathVector';
...
export class EfisVectors {
    ...
    public update(deltaTime: number): void {
        ...
        const engagedLateralMode = SimVar.GetSimVarValue('L:B77RS_FMA_LATERAL_MODE', 'Number') as LateralMode;
        const armedLateralMode = SimVar.GetSimVarValue('L:B77RS_FMA_LATERAL_ARMED', 'Enum');
    ...
    private transmit(vectors: PathVector[], vectorsGroup: EfisVectorsGroup, side: EfisSide): void {
        this.syncer.sendEvent(`B77RS_EFIS_VECTORS_${side}_${EfisVectorsGroup[vectorsGroup]}`, vectors);
    }
}
