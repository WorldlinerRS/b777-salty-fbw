...
export class LnavDriver implements GuidanceComponent {
    ...
    update(_: number): void {
        ...
        if (geometry && geometry.legs.size > 0) {
            ...
            SimVar.SetSimVarValue('L:B77RS_GPS_WP_DISTANCE', 'nautical miles', dtg ?? 0);
            ...
            const flightPhase = SimVar.GetSimVarValue('L:B77RS_FMGC_FLIGHT_PHASE', 'Enum') as FmgcFlightPhase;
    ...
    private updateEfisData(activeLeg: Leg, gs: Knots) {
        ...
        const efisBearing = termination ? B77RS_Util.trueToMagnetic(
