...
import { GuidanceComponent } from '@aims/guidance/GuidanceComponent';
import { PseudoWaypoint, PseudoWaypointSequencingAction } from '@aims/guidance/PsuedoWaypoint';
import { VnavConfig, VnavDescentMode } from '@aims/guidance/vnav/VnavConfig';
...
import { WaypointStats } from '@aims/flightplanning/data/flightplan';
...
export class PseudoWaypoints implements GuidanceComponent {
    ...
    sequencePseudoWaypoint(pseudoWaypoint: PseudoWaypoint): void {
        ...
        case PseudoWaypointSequencingAction.APPROACH_PHASE_AUTO_ENGAGE:
            const apLateralMode = SimVar.GetSimVarValue('L:B77RS_FMA_LATERAL_MODE', 'Number');
            ...
            if (agl < 9500 && (apLateralMode === LateralMode.NAV || apLateralMode === LateralMode.LOC_CPT || apLateralMode === LateralMode.LOC_TRACK)) {
                ...
                SimVar.SetSimVarValue('L:B77RS_FM_ENABLE_APPROACH_PHASE', 'Bool', true).then(() => [
                    setTimeout(() => {
                        SimVar.SetSimVarValue('L:B77RS_FM_ENABLE_APPROACH_PHASE', 'Bool', false);
                    }, 5_000),
                ]);
            }
            break;
