...
export class PopUp {
     ...
     */
    constructor() {
        const title = 'B77RS POPUP';
        const time = new Date().getTime();
        this.popupListener = undefined;
        this.params = {
            __Type: 'SNotificationParams',
            buttons: [new NotificationButton('TT:MENU.YES', `B77RS_POP_${title}_${time}_YES`), new NotificationButton('TT:MENU.NO', `B77RS_POP_${title}_${time}_NO`)],
            style: 'normal',
    ...
    showInformation(title: string, message: string, style: 'small'| 'normal'| 'big'| 'big-help', callback: () => void): void {
        ...
        this.params.buttons = [new NotificationButton('TT:MENU.OK', `B77RS_POP_${this.params.id}_YES`)];
