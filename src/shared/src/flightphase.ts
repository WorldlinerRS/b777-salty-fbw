...
export function isReady(): boolean {
    return SimVar.GetSimVarValue('L:B77RS_IS_READY', 'number') === 1;
}
...
export function isOnGround(): boolean {
    return SimVar.GetSimVarValue('L:B77RS_LGCIU_1_NOSE_GEAR_COMPRESSED', 'bool')
        || SimVar.GetSimVarValue('L:B77RS_LGCIU_2_NOSE_GEAR_COMPRESSED', 'bool');
}

function isEngineOn(index: number): boolean {
    return SimVar.GetSimVarValue(`L:B77RS_ENGINE_N2:${index}`, 'number') > 20;
}

function isEngineOnTakeOffThrust(index: number): boolean {
    return SimVar.GetSimVarValue(`L:B77RS_ENGINE_N1:${index}`, 'number') >= 70;
}
...
export function getAutopilotVerticalMode(): VerticalMode {
    return SimVar.GetSimVarValue('L:B77RS_FMA_VERTICAL_MODE', 'Enum');
}
