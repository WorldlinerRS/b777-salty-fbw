...
export class NXDataStore {
    ...
    static set(key: string, val: string): void {
        SetStoredData(`B77RS_${key}`, val);
        this.listener.triggerToAllSubscribers('B77RS_NXDATASTORE_UPDATE', key, val);
    }

    static subscribe(key: string, callback: SubscribeCallback): SubscribeCancellation {
        return Coherent.on('B77RS_NXDATASTORE_UPDATE', (updatedKey: string, value: string) => {
